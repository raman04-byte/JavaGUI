...
public class ScreenManager {
    ...
    public DisplayMode findFirstCompatablMode(DisplayMode modes[]) {
        DisplayMode goodmoMode[] = vc.getDisplayModes();
        for (int x = 0; x < modes.length; x++) {
            for (int y = 0; y < goodmoMode.length; y++) {
                if (displayModeMatch(modes[x], goodmoMode[y])) {
                    return modes[x];
                }
            }
        }
    ...
    // Creating images compatible for monitor 
